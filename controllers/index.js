                        ...
                        dao.q('portfolio_theme_settings', opts, function(err, settings) {
                            ...
                            self.ts.load('elements/callout', function(err, template) {
                                ...
                                mediaLoader.start(settings.page_layout, function(err, layout) {
                                    ...
                                    //agregando opciones de la busqueda de las noticias
                                    var optsNoticias = {
                                        $orderby: {
                                            created: -1
                                        }
                                    ...
                                    dao.q('article', optsNoticias, function(err, notSetind) {
                                        //variable que contiene  las noticias 
                                        var noticias = '';
                                        if( notSetind.length > 0 ){
                                            noticias = '<div class="capaNoticias">';
                                            noticias += '<img class="tituloNoticias" src="public/hayVacantes/img/headernoticias.png" alt="">';
                                            for (var i in notSetind ) {
                                                noticias += '<div class="capa2Noticias">';
                                                noticias += '<div class="noticias">' + 
                                                        '<h3>' + notSetind[i].headline + '</h3>' +
                                                        notSetind[i].article_layout + 
                                                        '<p><a href="article/' + notSetind[i].url + '">Leer mas...</a></p>' +
                                                    '</div>';
                                                noticias += '</div>';
                                            };
                                            noticias += '</div>';
                                        ...
                                        content.content = content.content.split('^layout^').join(( layout + noticias ));
